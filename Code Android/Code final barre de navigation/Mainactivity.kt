...
import androidx.appcompat.widget.Toolbar // Pour la barre d'outils
...
class MainActivity : AppCompatActivity() {
    ...
    // Variables pour la navigation drawer
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // DrawerLayout est maintenant la vue racine
        ...
        // === Conteneur principal pour le contenu du piano (hors navigation drawer) ===
        ...
        val toolbar = Toolbar(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 56f, resources.displayMetrics).toInt() // Hauteur standard de la Toolbar
            )
            title = "Piano Bluetooth"
            setTitleTextColor(Color.WHITE)
            setBackgroundColor(ContextCompat.getColor(context, R.color.purple_500)) // Remplacez par votre couleur
            setNavigationIcon(R.drawable.baseline_menu_24) // Icône de menu pour ouvrir le tiroir
            setNavigationOnClickListener {
                drawerLayout.openDrawer(Gravity.START) // Ouvre le tiroir au clic sur l'icône
            }
        }
        mainContentLayout.addView(toolbar)

        // === Création d'un ScrollView pour permettre le défilement vertical ===
        val scrollView = ScrollView(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.MATCH_PARENT // Prend le reste de la hauteur
            )
        }
        ...
        mainContentLayout.addView(scrollView) // Ajoute le ScrollView au contenu principal
        drawerLayout.addView(mainContentLayout) // Ajoute le contenu principal au DrawerLayout

        // === Création de la NavigationView (le menu latéral) ===
        navigationView = NavigationView(this).apply {
            layoutParams = DrawerLayout.LayoutParams(
                DrawerLayout.LayoutParams.WRAP_CONTENT,
                DrawerLayout.LayoutParams.MATCH_PARENT
            ).apply {
                gravity = Gravity.START // Le menu apparaît à gauche
            }
            // Ajouter les éléments de menu
            menu.add(0, MENU_ITEM_BLUETOOTH, 0, "Connecter Bluetooth").setIcon(R.drawable.baseline_bluetooth_24)
            menu.add(0, MENU_ITEM_TOGGLE_LABELS, 1, "Afficher/Masquer Notes").setIcon(R.drawable.baseline_music_note_24)
            // Vous pouvez ajouter d'autres éléments de menu ici si nécessaire (ex: Home, Page1, Page2)

            setNavigationItemSelectedListener { item ->
                when (item.itemId) {
                    MENU_ITEM_BLUETOOTH -> {
                        checkAndRequestBluetoothPermissions() // Lance le processus Bluetooth
                        drawerLayout.closeDrawer(Gravity.START) // Ferme le tiroir
                        true
                    }
        ...
        drawerLayout.addView(navigationView) // Ajoute la NavigationView au DrawerLayout

        setContentView(drawerLayout) // Définit le DrawerLayout comme vue principale
        ...
        // Le décalage vertical des touches noires ne dépend plus du bouton de menu,
        // mais doit toujours prendre en compte la Toolbar si les touches commencent juste en dessous.
        // Puisque les touches sont dans un ScrollView et que la Toolbar est au-dessus,
        // il n'y a pas de décalage supplémentaire spécifique à la Toolbar pour les touches noires.
        ...
        for ((position, noteName) in blackNotes) {
            val blackKey = Button(this).apply {
                ...
                layoutParams = FrameLayout.LayoutParams(
                    ...
                    topMargin = ((position - 1) * (keyHeightPx + (2 * keyMarginPx))) + blackKeyVerticalOffsetPx
                }
